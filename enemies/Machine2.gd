...
onready var bullet = preload("res://enemies/Projectiles/Mbomb.tscn")
...
var attacking = false
var ready = true
var general_delay = 0.1

var flip_tired_delay = 0
...
func die():
	...
	get_node("Enemy_Health_bar").visible = false
	
	yield(get_tree().create_timer(8), "timeout")
	$AnimatedSprite.animation = "Death"
	$AnimatedSprite.playing = false
	$AnimatedSprite.frame = 19
...
func attack():
	...
	if  randi()%3 == 0:
		...
		yield(get_tree().create_timer(0.705), "timeout")
		dodgeable = true
		...
		yield(get_tree().create_timer(0.605), "timeout")
		dodgeable = true
		yield(get_tree().create_timer(0.1), "timeout")
		hitting = true
		charging = false
		...
		for i in range(1):
			...
			var r = Vector2((randi()%200-100),(randi()%200-100))

			
			b.transform = $muzzle/right.global_transform
			b.direction = (PlayerData.player.position - position + r).normalized()	
			b.right = 1

			b = bullet.instance()		
			get_parent().add_child(b)
			r = Vector2((randi()%200-100),(randi()%200-100))
...
func _on_Shoot_timeout():
	...
	if (state == DEAD):
		...
		if position.distance_to(player.position) < RANGE and position.distance_to(player.position) > 170:
			print("Shooting...")
			$AnimatedSprite.play("Shoot")
			shooting = true
			for i in range(1):
