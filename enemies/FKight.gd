...
onready var bullet = preload("res://enemies/Projectiles/Boss7_Wave.tscn")
...
func attack():
	...
	if  randi()%2:
		...
		charging = true
		dodgeable = false
		hurt.attck(facing_right, 2.5, 10)
		yield(get_tree().create_timer(0.1), "timeout")
		hurt.attck(facing_right, 2.5, 10)
		yield(get_tree().create_timer(0.1), "timeout")
		hurt.attck(facing_right, 2.5, 10)
