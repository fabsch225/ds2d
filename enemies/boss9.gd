...
onready var bullet = preload("res://enemies/Projectiles/boss9_spell.tscn")
...
func _physics_process(_delta):
	...
	if (target != null):
		...
		if (hitting or charging):
			...
			Nav.add_request(Vector2(position.x-72*c, position.y),target.position,self)
		line.points = path

		if (len(path) > 1):
			velocity.x = Vector2(position.x-72*c, position.y).direction_to(path[1]).normalized().x * speed
		...
		if (attacking):
			...
			if (d > attack_range and d < reach):
				velocity.x = abs(position.x - player.position.x) * 2.3
